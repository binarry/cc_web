    ...
    imageCropper.prototype = {
        ...
        init: function(image, settings) {
            ...
            $('.zoom-out-button,.zoom-in-button', this.wrapper).click(function() {
                ...
                //如果缩放级别小于 临界尺寸  则设置缩放级别为  临界尺寸
                var widthLevel = context.formatNumber(context.settings.width/context.img.width, 3);
                var heightLevel = context.formatNumber(context.settings.height/context.img.height, 3);
                var criticalLevel = Math.max(widthLevel, heightLevel);// 临界尺寸，即原图不小于截取框的最小尺寸
                if (tempZoomLevel < criticalLevel) {
                	tempZoomLevel = criticalLevel;
                }
            ...
            $('.zoom-scroller', this.wrapper).draggable({
                ...
                drag: function(event, ui) {
                    ...
                    if (tempZoomLevel < criticalLevel) {
                    	tempZoomLevel = criticalLevel;
                    	cancelZoomScroll = true;//需重置滚动条的值
                    }
                    //如果缩放级别超出范围 或者 缩放导致图片右下角没在截取框内 则取消缩放
        ...
        zoom: function(width, height) {
            ...
            $('.drag-containment', this.wrapper).css({
                left: this.settings.cropFrameRect.left + this.settings.width - this.settings.zoomLevel * this.img.width + 'px',
                top: this.settings.cropFrameRect.top + this.settings.height - this.settings.zoomLevel * this.img.height + 'px',
