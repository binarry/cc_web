!function(a) {
	...
	var b, c = "function" == typeof define && define.amd, d = function(b) {
		...
		options : {
			...
			lang : "en-US",
			direction : null,
			toolbar : [["style", ["style"]], ["font", ["bold", "italic", "underline", "superscript", "subscript", "strikethrough", "clear"]], ["fontname", ["fontname"]], ["color", ["color"]], ["para", ["ul", "ol", "paragraph"]],
					["height", ["height"]], ["table", ["table"]], ["insert", ["link", "picture", "video", "hr"]], ["view", ["fullscreen", "codeview"]], ["help", ["help"]]],
			airMode : !1,
			...
			colors : [["#000000", "#424242", "#636363", "#9C9C94", "#CEC6CE", "#EFEFEF", "#F7F7F7", "#FFFFFF"], ["#FF0000", "#FF9C00", "#FFFF00", "#00FF00", "#00FFFF", "#0000FF", "#9C00FF", "#FF00FF"],
					["#F7C6CE", "#FFE7CE", "#FFEFC6", "#D6EFD6", "#CEDEE7", "#CEE7F7", "#D6D6E7", "#E7D6DE"], ["#E79C9C", "#FFC69C", "#FFE79C", "#B5D6A5", "#A5C6CE", "#9CC6EF", "#B5A5D6", "#D6A5BD"],
					["#E76363", "#F7AD6B", "#FFD663", "#94BD7B", "#73A5AD", "#6BADDE", "#8C7BC6", "#C67BA5"], ["#CE0000", "#E79439", "#EFC631", "#6BA54A", "#4A7B8C", "#3984C6", "#634AA5", "#A54A7B"],
					["#9C0000", "#B56308", "#BD9400", "#397B21", "#104A5A", "#085294", "#311873", "#731842"], ["#630000", "#7B3900", "#846300", "#295218", "#083139", "#003163", "#21104A", "#4A1031"]],
		...
		this.stylePara = function(b, c) {
			...
			var e = a(h.isText(c.sc) ? c.sc.parentNode : c.sc), f = ["font-family", "font-size", "text-align", "list-style-type", "line-height"], g = b(e, f) || {};
			if (g["font-size"] = parseInt(g["font-size"], 10), g["font-bold"] = document.queryCommandState("bold") ? "bold" : "normal", g["font-italic"] = document.queryCommandState("italic") ? "italic" : "normal", g["font-underline"] = document
					.queryCommandState("underline") ? "underline" : "normal", g["font-strikethrough"] = document.queryCommandState("strikeThrough") ? "strikethrough" : "normal", g["font-superscript"] = document.queryCommandState("superscript")
					? "superscript"
					: "normal", g["font-subscript"] = document.queryCommandState("subscript") ? "subscript" : "normal", c.isOnList()) {
				var i = ["circle", "disc", "disc-leading-zero", "square"], j = a.inArray(g["list-style-type"], i) > -1;
		...
		var b = !!document.createRange, c = function(a, b) {
			var c, d, e = a.parentElement(), f = document.body.createTextRange(), i = g.from(e.childNodes);
			for (c = 0; c < i.length; c++)
				if (!h.isText(i[c])) {
					if (f.moveToElementText(i[c]), f.compareEndPoints("StartToStart", a) >= 0)
						break;
					d = i[c]
				}
			...
			var b = function(a, c) {
				var d, e;
				if (h.isText(a)) {
					var i = h.listPrev(a, f.not(h.isText)), j = g.last(i).previousSibling;
					d = j || a.parentNode, c += g.sum(g.tail(i), h.length), e = !j
				} else {
					if (d = a.childNodes[c] || a, h.isText(d))
						return b(d, c);
					c = 0, e = !1
				}
				return {
			...
			}, c = document.body.createTextRange(), d = b(a.cont, a.offset);
			...
			this.getBPs = function() {
				...
				b = b || f.ok;
				var d = h.listBetween(c, j), e = g.compact(a.map(d, function(a) {
				...
				var b = h.ancestor(c, a), d = h.ancestor(j, a);
				if (!b && !d)
					return new e(c, i, j, k);
				var f = this.getBPs();
				...
				if (this.isCollapsed())
					return this;
				var b = this.splitText(), c = h.prevBP(b.getStartBP());
		...
		return {
			...
			createFromBookmark : function(a, b) {
				var c = h.fromOffsetPath(a, b.s.path), d = b.s.offset, f = h.fromOffsetPath(a, b.e.path), g = b.e.offset;
				return new e(c, d, f, g)
			}
		}
	}(), n = function() {
		this.tab = function(a, b) {
			var c = h.ancestor(a.commonAncestor(), h.isCell), d = h.ancestor(c, h.isTable), e = h.listDescendant(d, h.isCell), f = g[b ? "prev" : "next"](e, c);
			f && m.create(f, 0).select()
		}, this.createTable = function(b, c) {
			for (var d, e = [], f = 0; b > f; f++)
				e.push("<td>" + h.blank + "</td>");
			d = e.join("");
			for (var g, i = [], j = 0; c > j; j++)
				i.push("<tr>" + d + "</tr>");
			g = i.join("");
		...
		}, f = ["bold", "italic", "underline", "strikethrough", "superscript", "subscript", "justifyLeft", "justifyCenter", "justifyRight", "justifyFull", "insertOrderedList", "insertUnorderedList", "indent", "outdent", "formatBlock",
				"removeFormat", "backColor", "foreColor", "insertHorizontalRule", "fontName"], i = 0, k = f.length; k > i; i++)
			this[f[i]] = function(a) {
				return function(b, c) {
		...
		this.tab = function(a, b) {
			...
			d(b);
			var e, f = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/, g = c.match(f), h = /\/\/instagram.com\/p\/(.[a-zA-Z0-9]*)/, i = c.match(h), j = /\/\/vine.co\/v\/(.[a-zA-Z0-9]*)/, k = c.match(j), l = /\/\/(player.)?vimeo.com\/([a-z]*\/)*([0-9]{6,11})[?]?.*/, n = c
					.match(l), o = /.+dailymotion.com\/(video|hub)\/([^_]+)[^#]*(#video=([^_&]+))?/, p = c.match(o), q = /\/\/v\.youku\.com\/v_show\/id_(\w+)\.html/, r = c.match(q);
			if (g && 11 === g[2].length) {
				var s = g[2];
				e = a("<iframe>").attr("src", "//www.youtube.com/embed/" + s).attr("width", "640").attr("height", "360")
			} else
				i && i[0].length ? e = a("<iframe>").attr("src", i[0] + "/embed/").attr("width", "612").attr("height", "710").attr("scrolling", "no").attr("allowtransparency", "true") : k && k[0].length ? e = a("<iframe>").attr("src",
						k[0] + "/embed/simple").attr("width", "600").attr("height", "600").attr("class", "vine-embed") : n && n[3].length ? e = a("<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>").attr("src",
						"//player.vimeo.com/video/" + n[3]).attr("width", "640").attr("height", "360") : p && p[2].length ? e = a("<iframe>").attr("src", "//www.dailymotion.com/embed/video/" + p[2]).attr("width", "640").attr("height", "360") : r
						&& r[1].length && (e = a("<iframe webkitallowfullscreen mozallowfullscreen allowfullscreen>").attr("height", "498").attr("width", "510").attr("src", "//player.youku.com/embed/" + r[1]));
		...
		this.fontSize = function(a, b) {
			...
			var f = c.url, g = c.text, h = c.newWindow, i = c.range;
			...
			b.focus();
			var c = m.create().expand(h.isAnchor), d = a(g.head(c.nodes(h.isAnchor)));
			return {
				range : c,
				text : c.toString(),
	...
	var v = function() {
		var c = a(window), d = a(document), f = a("html, body"), i = new o, l = new r, m = new s, n = new t, q = new u, v = function(b) {
			...
			showLinkDialog : function(a) {
				var b = a.editor(), c = a.dialog(), d = a.editable(), e = i.getLinkInfo(d), f = b.data("options");
			...
			showVideoDialog : function(a) {
				var b = a.dialog(), c = a.editable(), d = i.getVideoInfo(c);
			...
			fullscreen : function(a) {
				var b = a.editor(), d = a.toolbar(), e = a.editable(), g = a.codable(), h = b.data("options"), i = function(a) {
			...
			codeview : function(a) {
				var c, d, f = a.editor(), g = a.toolbar(), i = a.editable(), j = a.codable(), k = a.popover(), n = f.data("options");
				f.toggleClass("codeview");
				var o = f.hasClass("codeview");
				o ? (j.val(i.html()), j.height(i.height()), l.deactivate(g), m.hide(k), j.focus(), e.hasCodeMirror
						&& (c = b.fromTextArea(j[0], n.codemirror), n.codemirror.tern && (d = new b.TernServer(n.codemirror.tern), c.ternServer = d, c.on("cursorActivity", function(a) {
									d.updateArgHints(a)
								...
								}), j.data("cmEditor", c))) : (e.hasCodeMirror && (c = j.data("cmEditor"), j.val(c.getValue()), c.toTextArea()), i.html(j.val() || h.emptyPara), i.height(n.height ? j.height() : "auto"), l.activate(g), i.focus()), l
						.updateCodeview(a.toolbar(), o)
			}
		}, y = function(a) {
			h.isImg(a.target) && a.preventDefault()
		}, z = function(a) {
			setTimeout(function() {
						var b = v(a.currentTarget || a.target), c = i.currentStyle(a.target);
						if (c) {
			...
			if (b && b.items && b.items.length) {
				var c = v(a.currentTarget || a.target), d = g.head(b.items), e = "file" === d.kind && -1 !== d.type.indexOf("image/");
				e && w(c.editable(), [d.getAsFile()])
			}
		}, C = function(b) {
			if (h.isControlSizing(b.target)) {
				b.preventDefault(), b.stopPropagation();
				var c = v(b.target), e = c.handle(), f = c.popover(), g = c.editable(), j = c.editor(), k = e.find(".note-control-selection").data("target"), l = a(k), o = l.offset(), p = d.scrollTop(), q = j.data("options").airMode;
			...
			var c = a(b.target).closest("[data-event]");
			if (c.length) {
				var d = c.attr("data-event"), e = c.attr("data-value"), f = v(b.target);
				b.preventDefault();
				var h;
				...
				if (-1 !== a.inArray(d, ["backColor", "foreColor"])) {
					var n = f.editor().data("options", n), o = n.airMode ? m : l;
			...
			var b = v(a.target).editable(), c = b.offset().top - d.scrollTop(), e = v(a.currentTarget || a.target), f = e.editor().data("options");
			d.on("mousemove", function(a) {
						var d = a.clientY - (c + F);
		...
		this.bindKeyMap = function(a, b) {
			...
			this.bindKeyMap(a, b.keyMap[e.isMac ? "mac" : "pc"]), a.editable.on("mousedown", y), a.editable.on("keyup mouseup", z), a.editable.on("scroll", A), a.editable.on("paste", B), a.handle.on("mousedown", C), a.popover.on("click", E), a.popover
					.on("mousedown", D), b.airMode || (b.disableDragAndDrop || J(a), a.toolbar.on("click", E), a.toolbar.on("mousedown", D), b.disableResizeEditor || a.statusbar.on("mousedown", G));
			...
			if (d.css({
					...
					}), b.onfocus && a.editable.focus(b.onfocus), b.onblur && a.editable.blur(b.onblur), b.onkeyup && a.editable.keyup(b.onkeyup), b.onkeydown && a.editable.keydown(b.onkeydown), b.onpaste && a.editable.on("paste", b.onpaste), b.onToolbarClick
		...
		var b = function(a, b) {
			...
			return '<button type="button" class="btn btn-default btn-sm btn-small' + (f ? " " + f : "") + (g ? " dropdown-toggle" : "") + '"' + (g ? ' data-toggle="dropdown"' : "") + (e ? ' title="' + e + '"' : "")
					+ (c ? ' data-event="' + c + '"' : "") + (d ? " data-value='" + d + "'" : "") + ' tabindex="-1">' + a + (g ? ' <span class="caret"></span>' : "") + "</button>" + (g || "")
		}, c = function(a, c) {
			var d = '<i class="' + a + '"></i>';
			return b(d, c)
		}, d = function(a, b) {
			...
			return '<div class="' + a + ' modal" aria-hidden="false"><div class="modal-dialog"><div class="modal-content">'
					+ (b ? '<div class="modal-header"><button type="button" class="close" aria-hidden="true" tabindex="-1">&times;</button><h4 class="modal-title">' + b + "</h4></div>" : "")
					+ '<form class="note-modal-form"><div class="modal-body"><div class="row-fluid">' + c + "</div></div>" + (d ? '<div class="modal-footer">' + d + "</div>" : "") + "</form></div></div></div>"
		}, i = {
			picture : function(a) {
			...
			color : function(a) {
				var c = '<i class="fa fa-font icon-font" style="color:black;background-color:yellow;"></i>', d = b(c, {
						...
						}), e = '<ul class="dropdown-menu"><li><div class="btn-group"><div class="note-palette-title">' + a.color.background + '</div><div class="note-color-reset" data-event="backColor" data-value="inherit" title="'
						+ a.color.transparent + '">' + a.color.setTransparent + '</div><div class="note-color-palette" data-target-event="backColor"></div></div><div class="btn-group"><div class="note-palette-title">' + a.color.foreground
						+ '</div><div class="note-color-reset" data-event="foreColor" data-value="inherit" title="' + a.color.reset + '">' + a.color.resetToDefault
						+ '</div><div class="note-color-palette" data-target-event="foreColor"></div></div></li></ul>', f = b("", {
							title : a.color.more,
							dropdown : e
						});
				return d + f
			...
			var f = function() {
				...
				for (var b = "", c = 0, f = e.airPopover.length; f > c; c++) {
					...
					for (var h = 0, j = g[1].length; j > h; h++)
						b += i[g[1][h]](a, e);
					b += "</div>"
				}
			...
			var b = "<tr><td>⌘ + B</td><td>" + a.font.bold + "</td></tr><tr><td>⌘ + I</td><td>" + a.font.italic + "</td></tr><tr><td>⌘ + U</td><td>" + a.font.underline + "</td></tr><tr><td>⌘ + ⇧ + S</td><td>" + a.font.strikethrough
					+ "</td></tr><tr><td>⌘ + \\</td><td>" + a.font.clear + "</td></tr>";
			return l(a.shortcut.textFormatting, b)
		}, n = function(a) {
			var b = "<tr><td>⌘ + Z</td><td>" + a.history.undo + "</td></tr><tr><td>⌘ + ⇧ + Z</td><td>" + a.history.redo + "</td></tr><tr><td>⌘ + ]</td><td>" + a.paragraph.indent + "</td></tr><tr><td>⌘ + [</td><td>" + a.paragraph.outdent
					+ "</td></tr><tr><td>⌘ + ENTER</td><td>" + a.hr.insert + "</td></tr>";
			return l(a.shortcut.action, b)
		}, o = function(a) {
			var b = "<tr><td>⌘ + ⇧ + L</td><td>" + a.paragraph.left + "</td></tr><tr><td>⌘ + ⇧ + E</td><td>" + a.paragraph.center + "</td></tr><tr><td>⌘ + ⇧ + R</td><td>" + a.paragraph.right + "</td></tr><tr><td>⌘ + ⇧ + J</td><td>"
					+ a.paragraph.justify + "</td></tr><tr><td>⌘ + ⇧ + NUM7</td><td>" + a.lists.ordered + "</td></tr><tr><td>⌘ + ⇧ + NUM8</td><td>" + a.lists.unordered + "</td></tr>";
			return l(a.shortcut.paragraphFormatting, b)
		}, p = function(a) {
			var b = "<tr><td>⌘ + NUM0</td><td>" + a.style.normal + "</td></tr><tr><td>⌘ + NUM1</td><td>" + a.style.h1 + "</td></tr><tr><td>⌘ + NUM2</td><td>" + a.style.h2 + "</td></tr><tr><td>⌘ + NUM3</td><td>" + a.style.h3
					+ "</td></tr><tr><td>⌘ + NUM4</td><td>" + a.style.h4 + "</td></tr><tr><td>⌘ + NUM5</td><td>" + a.style.h5 + "</td></tr><tr><td>⌘ + NUM6</td><td>" + a.style.h6 + "</td></tr>";
			...
			var c = function() {
				var b = "<h5>" + a.image.selectFromFiles + '</h5><input class="note-image-input" type="file" name="files" accept="image/*" /><h5>' + a.image.url + '</h5><input class="note-image-url form-control span12" type="text" />', c = '<button href="#" class="btn btn-primary note-image-btn disabled" disabled>'
						+ a.image.insert + "</button>";
				return g("note-image-dialog", a.image.insert, b, c)
			}, d = function() {
				var c = '<div class="form-group"><label>' + a.link.textToDisplay + '</label><input class="note-link-text form-control span12" type="text" /></div><div class="form-group"><label>' + a.link.url
						+ '</label><input class="note-link-url form-control span12" type="text" /></div>' + (b.disableLinkTarget ? "" : '<div class="checkbox"><label><input type="checkbox" checked> ' + a.link.openInNewWindow + "</label></div>"), d = '<button href="#" class="btn btn-primary note-link-btn disabled" disabled>'
						+ a.link.insert + "</button>";
				return g("note-link-dialog", a.link.insert, c, d)
			}, f = function() {
				var b = '<div class="form-group"><label>' + a.video.url + '</label>&nbsp;<small class="text-muted">' + a.video.providers + '</small><input class="note-video-url form-control span12" type="text" /></div>', c = '<button href="#" class="btn btn-primary note-video-btn disabled" disabled>'
						+ a.video.insert + "</button>";
				return g("note-video-dialog", a.video.insert, b, c)
			}, h = function() {
				var c = '<a class="modal-close pull-right" aria-hidden="true" tabindex="-1">'
						+ a.shortcut.close
						+ '</a><div class="title">'
						+ a.shortcut.shortcuts
						+ "</div>"
						+ (e.isMac ? r(a, b) : s(r(a, b)))
						+ '<p class="text-center"><a href="//hackerwins.github.io/summernote/" target="_blank">Summernote 0.5.3</a> · <a href="//github.com/HackerWins/summernote" target="_blank">Project</a> · <a href="//github.com/HackerWins/summernote/issues" target="_blank">Issues</a></p>';
		...
		this.createLayoutByAirMode = function(b, c) {
			...
			for (var l = a.summernote.lang[c.lang], m = "", n = 0, o = c.toolbar.length; o > n; n++) {
				...
				for (var r = 0, s = q.length; s > r; r++)
					a.isFunction(i[q[r]]) && (m += i[q[r]](l, c));
				m += "</div>"
			}
	...
	a.fn.extend({
				...
				destroy : function() {
					return this.each(function(b, c) {
								var d = a(c), e = x.layoutInfoFromHolder(d);
								if (e && e.editable) {
