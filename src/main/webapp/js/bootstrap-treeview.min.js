!function(a, b, c, d) {
	...
	f.defaults = {
		...
		_init : function(b) {
			b.data && ("string" == typeof b.data && (b.data = a.parseJSON(b.data)), this.tree = a.extend(!0, [], b.data), delete b.data), this.options = a.extend({}, f.defaults, b), this._setInitialLevels(this.tree, 0), this._destroy(), this
					._subscribeEvents(), this._render()
		},
		_unsubscribeEvents : function() {
			this.$element.off("click"), "function" == typeof this.options.onNodeSelected && this.$element.off("nodeSelected")
		},
		...
		_clickHandler : function(b) {
			...
			-1 != d.indexOf("click-expand") || -1 != d.indexOf("click-collapse") ? (this._toggleNodes(e), this._render()) : e && (this._isSelectable(e) ? this._setSelectedNode(e) : (this._toggleNodes(e), this._render()))
		},
		...
		_buildTree : function(b, c) {
			if (b) {
				...
				a.each(b, function(b, e) {
							...
							for (var f = a(d._template.item).addClass("node-" + d._elementId).addClass(e === d.selectedNode ? "node-selected" : "").attr("data-nodeid", e.nodeId).attr("style", d._buildStyleOverride(e)), g = 0; c - 1 > g; g++)
								f.append(d._template.indent);
							return f.append(e._nodes ? a(d._template.expandCollapseIcon).addClass("click-expand").addClass(d.options.expandIcon) : e.nodes ? a(d._template.expandCollapseIcon).addClass("click-collapse")
									.addClass(d.options.collapseIcon) : a(d._template.expandCollapseIcon).addClass(d.options.emptyIcon)), f.append(a(d._template.icon).addClass(e.icon ? e.icon : d.options.nodeIcon)), f.append(d.options.enableLinks
									? a(d._template.link).attr("href", e.href).append(e.text)
									: e.text), d.options.showTags && e.tags && a.each(e.tags, function(b, c) {
		...
		_buildStyleOverride : function(a) {
			var b = "";
			return this.options.highlightSelected && a === this.selectedNode ? b += "color:" + this.options.selectedColor + ";" : a.color && (b += "color:" + a.color + ";"), this.options.highlightSelected && a === this.selectedNode
					? b += "background-color:" + this.options.selectedBackColor + ";"
					: a.backColor && (b += "background-color:" + a.backColor + ";"), b
		},
		...
		_buildStyle : function() {
			...
			return this.options.color && (a += "color:" + this.options.color + ";"), this.options.backColor && (a += "background-color:" + this.options.backColor + ";"), this.options.showBorder ? this.options.borderColor
					&& (a += "border:1px solid " + this.options.borderColor + ";") : a += "border:none;", a += "}", this.options.onhoverColor && (a += ".node-" + this._elementId + ":hover{background-color:" + this.options.onhoverColor + ";}"), this._css
					+ a
		},
		_template : {
	...
	a.fn[e] = function(b, c) {
		return this.each(function() {
					...
					"string" == typeof b ? d ? a.isFunction(d[b]) && "_" !== b.charAt(0) ? ("string" == typeof c && (c = [c]), d[b].apply(d, c)) : g("No such method : " + b) : g("Not initialized, can not call method : " + b) : d ? d._init(b) : a
							.data(this, "plugin_" + e, new f(this, a.extend(!0, {}, b)))
				})
	}
