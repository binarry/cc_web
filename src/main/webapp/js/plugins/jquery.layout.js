...
(function($) {
	...
	function _25(_26, _27, _28) {
		...
		function _2b(dir) {
			...
			var p = $("<div></div>").appendTo(_26).panel({
						cls : "layout-expand",
						title : "&nbsp;",
						closed : true,
						doSize : false,
		...
		function _2d() {
			...
			if (_27 == "east") {
				return {
					resizeC : {
						width : _29.center.panel("options").width + _29["east"].panel("options").width - 28
					},
					expand : {
						left : cc.width() - _29["east"].panel("options").width
					},
					expandP : {
						top : _29["east"].panel("options").top,
						left : cc.width() - 28,
						width : 28,
						height : _29["center"].panel("options").height
					},
					collapse : {
						left : cc.width()
					}
				...
				if (_27 == "west") {
					return {
						resizeC : {
							width : _29.center.panel("options").width + _29["west"].panel("options").width - 28,
							left : 28
						},
						expand : {
							left : 0
						...
						expandP : {
							left : 0,
							top : _29["west"].panel("options").top,
							width : 28,
							height : _29["center"].panel("options").height
						},
						collapse : {
							left : -_29["west"].panel("options").width
						}
					};
				} else {
					if (_27 == "north") {
						...
						_29.east.panel("resize", {
									top : 28,
									height : hh
								});
						_29.west.panel("resize", {
									top : 28,
									height : hh
								});
						if (_b(_29.expandEast)) {
							_29.expandEast.panel("resize", {
										top : 28,
										height : hh
									});
						}
						if (_b(_29.expandWest)) {
							_29.expandWest.panel("resize", {
										top : 28,
										height : hh
									});
						}
						return {
							...
							collapse : {
								top : -_29["north"].panel("options").height
							}
						};
					} else {
						if (_27 == "south") {
							...
							return {
								...
								expand : {
									top : cc.height() - _29["south"].panel("options").height
								},
								expandP : {
									top : cc.height() - 28,
									left : 0,
		...
		function _35() {
			...
			if (_32 == "east" && _33.expandEast) {
				return {
					...
					expand : {
						left : cc.width() - _33["east"].panel("options").width
					}
				};
			} else {
				if (_32 == "west" && _33.expandWest) {
					return {
						collapse : {
							left : -_33["west"].panel("options").width
						},
						expand : {
							left : 0
						}
